#!/usr/bin/env bash
# apolo-quick-test.sh — Test rapido v3.5.5 (~15 segundos)
# FIX v3.5.5: grep patterns robustos para los 3 tests que fallaban
set -uo pipefail
...
echo -e "${BOLD}${GREEN}║  QUICK TEST apolo-dynamic-flow v3.5.5 (~15s)           ║${NC}"
echo -e "${BOLD}${GREEN}╚═══════════════════════════════════════════════════════╝${NC}"
...
# FIX v3.5.5: flow_verifier con grep robusto
...
python3 scripts/python/integration_validator.py validate --repo-root . 2>&1 | grep -qi "success\|overall\|handoff\|integration\|phase" && pass "integration_validator" || fail "integration"

# FIX v3.5.5: honesty con grep robusto
